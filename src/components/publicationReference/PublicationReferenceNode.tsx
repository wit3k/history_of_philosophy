...
const PublicationReferenceNode = (props: PublicationReferenceNodeProps) => {
  if (props.publicationReference.from && props.publicationReference.to) {
    ...
    if (props.positionEnd === props.positionStart) {
      ...
      points.push(
        new Coordinates(
          start.x + props.settings.boxSize * distanceFromFactor * cos05 * hdir * shrinkFactor,
          start.y + props.settings.boxSize * distanceFromFactor * vdir * isEqual,
        ),
      )
      ...
      points.push(new Coordinates(end.x, end.y - props.settings.boxSize * distanceToFactor * vdir))
